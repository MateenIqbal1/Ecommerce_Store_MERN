...
function Address({ setCurrentSelectedAddress, selectedId }) {
    ...
    function handleManageAddress(event) {
        ...
        if (addressList.length >= 3 && currentEditedId === null) {
            ...
            toast({ title: 'you can add max 3 addresses', className: 'bg-red-500 text-white !important' })
        ...
        currentEditedId !== null ? dispatch(editaAddress({ userId: user?.id, addressId: currentEditedId, formData })).then((data) => {
            if (data?.payload?.success) {
                ...
                setFormData(initialAddressFormData)
                toast({ title: 'address updated successfully', className: 'bg-green-500 text-white !important' })
            ...
            dispatch(addNewAddress({
                ...
                if (data?.payload?.success) {
                    dispatch(fetchAllAddresses(user?.id))
                    setFormData(initialAddressFormData)
                    toast({ title: 'address added successfully', className: 'bg-green-500 text-white !important' })
    ...
    function handleDeleteAddress(getCurrentAddress) {
        ...
        dispatch(deleteAddress({ userId: user?.id, addressId: getCurrentAddress._id })).then(data => {
            if (data?.payload?.success) {
                dispatch(fetchAllAddresses(user?.id))
                toast({ title: 'address deleted successfully', className: 'bg-red-500 text-white !important' })
    ...
    return (
        <Card>
            ...
            <CardContent className='space-y-3'>
                <CommonForm formControls={addressFormControls} formData={formData} setFormData={setFormData} buttonText={currentEditedId !== null ? 'Edit ' : 'Add '}
